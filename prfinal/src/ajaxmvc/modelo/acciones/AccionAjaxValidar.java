 ...
 * @version  1
 */
public class AccionAjaxValidar implements Accion {
	/**
	...
	public boolean ejecutar(HttpServletRequest request,
		...
		boolean estado = true;
		ProcesaDAOUsuario procesaBD = new ProcesaDAOUsuario(this.getDS(),this.archivoSql);
		...
		try {
			if (procesaBD.existeUsr(login)){
				if (procesaBD.hayFotogramaConcurso(request.getParameter("login"))){
					modelo.setRespuesta("hayconcurso");
					request.getSession().setAttribute("login", login);
				
							
				}else{
					modelo.setRespuesta("nohayconcurso");
